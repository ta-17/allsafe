export function createBarChart(data, containerId, selectedLevel2Category) {
    ...
    function flattenData(data) {
        ...
        for (const [level2, level3Map] of grouped) {
            if (
                level2 === 'Unexpected money' ||
                level2 === 'Threats and extortion'
            )
                continue
        ...
        if (
            ...
            const attemptsData = flattened.find(
                (d) =>
                    d.category_level2 ===
                    'Attempts to gain your personal information'
            )
    ...
    const bars = svg
        ...
        .on('mouseover', function (event, d) {
            if (selectedBar !== this) {
                // Only apply hover effect if this bar is not selected
            ...
            tooltip
                .style('opacity', 1)
                .html(
                    `Scam Type: ${d.category_level2}<br>Total Lost Amount: $${formatNumber.format(d.totalAmount)}`
                )
        ...
        .on('click', function (event, d) {
            ...
            if (selectedBar === this) {
                ...
                d3.select(this).style('opacity', 0.6) // Reset opacity
                ...
                if (selectedBar === null) {
                    d3.select(selectedBar).style('opacity', 0.8) // Reset opacity
    ...
    window.addEventListener('sunburstClick', function (event) {
        ...
        console.log(
            'Bar Chart Received Sunburst Click Event, level2Id:',
            level2Id
